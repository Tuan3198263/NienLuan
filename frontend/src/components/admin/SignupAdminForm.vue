    ...
    <form @submit.prevent="handleSubmit">
      ...
      <div class="mb-3">
        ...
        <input
          ...
          required
        />
        <small v-if="errors.verificationCode" class="text-danger">{{
      ...
      <!-- Nút đăng ký -->
      <button type="submit" class="btn btn-primary w-100">Đăng Ký Admin</button>
    </form>
...
export default {
  ...
  setup() {
    const form = ref({
      ...
      password: "",
      phone: "",
      verificationCode: "", // Thêm mã xác thực vào form
    });
    ...
    const validateForm = () => {
      ...
      if (!form.value.verificationCode) {
        errors.value.verificationCode = "Mã xác thực là bắt buộc.";
        isValid = false;
      }
    ...
    const handleSubmit = async () => {
      ...
      try {
        const response = await axios.post(
          "http://localhost:3000/api/auth/register-admin", // Gọi API register-admin
          form.value
        );
        toast.success("Đăng ký admin thành công");
        router.push("/login");
      } catch (error) {
        toast.error(error.response?.data?.message || "Lỗi đăng ký admin");
      }
    };
